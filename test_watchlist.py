...
class WatchlistTestCase(unittest.TestCase):
    ...
    def test_update_item(self):
        ...
        # 测试更新条目操作，但电影标题为空
        response = self.client.post(
            "/movie/edit/1", data=dict(title="", year="2023"), follow_redirects=True
        )
        data = response.get_data(as_text=True)
    ...
    def test_login(self):
        ...
        response = self.client.post(
            "/login",
            data=dict(username="wrong", password="123456"),
            follow_redirects=True,
        )
        data = response.get_data(as_text=True)
        self.assertNotIn("Login success.", data)
        ...
        # 测试使用空用户名登录
        response = self.client.post(
            "/login", data=dict(username="", password="123456"), follow_redirects=True
        )
        data = response.get_data(as_text=True)
        self.assertNotIn("Login success.", data)
        self.assertIn("Invalid input.", data)
    ...
    def test_admin_command(self):
        ...
        result = self.runner.invoke(
            args=["admin", "--username", "qing", "--password", "123"]
        )
    ...
    def test_admin_command_update(self):
        result = self.runner.invoke(
            args=["admin", "--username", "clear", "--password", "456"]
        )
